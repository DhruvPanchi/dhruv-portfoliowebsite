...
// Renders only the icons that actually have a link behind them
const ProjectLinks = ({
  gitLink,
  youtubeLink,
  liveLink,
...
// Image block that only becomes a link when a live link exists
const ProjectImage = ({
  image,
  liveLink,
}: {
  ...
  return liveLink ? (
    <a className={sharedClass} href={liveLink} target="_blank" rel="noreferrer">
      {content}
    </a>
  ) : (
